exports.run = (client, message, args, member) => {
  ...
  if(member.roles.has('514465659250278429')) {
    ...
    if(args.length <= 0) {
      message.author.send('Thank you for starting the verification process. Please provide the link to your gamekit profile by using `>link [url]` (without []) here.').catch(err => {});
